    ...
    public ResponseEntity<?> placeOrder(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ErrorResponse().message(e.getMessage()).error("INVALID_ORDER"));
        } catch (NoSuchElementException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(new ErrorResponse().message(e.getMessage()).error("NOT_FOUND"));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ErrorResponse().message("Failed to place order: " + e.getMessage()).error("SERVER_ERROR"));
        }
    }
        ...
        try {
            ...
            if (status != null && !status.isEmpty()) {
                ...
                orders = orders.stream()
                    .filter(o -> o.getStatus().toString().equalsIgnoreCase(statusFilter))
                    .toList();
            }
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ErrorResponse().message("Invalid account ID format").error("INVALID_ID"));
        } catch (NoSuchElementException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(new ErrorResponse().message(e.getMessage()).error("NOT_FOUND"));
        }
